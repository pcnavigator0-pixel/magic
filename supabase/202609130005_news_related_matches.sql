    ...
    and not exists (
      ...
      where not (
        (block.item->>'type' = 'paragraph'
          ...
          and (not (block.item ? 'related_match_ids') or (jsonb_typeof(block.item->'related_match_ids') = 'array' and not exists (select 1 from jsonb_array_elements(block.item->'related_match_ids') as related(value) where jsonb_typeof(related.value) <> 'string'))))
        or (block.item->>'type' = 'list'
          and jsonb_typeof(block.item->'ordered') = 'boolean'
          and jsonb_typeof(block.item->'items') = 'array'
          and not exists (select 1 from jsonb_array_elements(block.item->'items') as item(value) where jsonb_typeof(item.value) <> 'string'))
